...
#值函数数据集：mpc_dataset5--30--20w.npz
#神经值函数：value_function_model5--30--20w.pth
import numpy as np

sim_time = 200                             # Total simulation time steps
Ts = 0.1                                   # Sampling time [s]
T_horizon =5                    # Prediction horizon time steps
gamma = 0.2   #0.2                             # CBF parameter in [0,1]
safety_dist = 0.03                         # Safety distance
x0 = np.array([0, 0, 0])                   # Initial state
friction=1
# Actuator limits
v_limit = 0.26  #0.5                           # Linear velocity limit
omega_limit = 1.8#1.8                          # Angular velocity limit
